...
SET INSTALLVER=4.2.5.2
IF %REGVALUEVER%==%INSTALLVER% GOTO :CHECK_UPDATE_CONFIG

REM Install the agent
start /wait msiexec /i AppDynamics\dotNetAgentSetup64.msi AD_AGENT_ENVIRONMENT=Azure AD_AZUREROLENAME=%RoleName% AD_AZUREROLEINSTANCEID=%RoleInstanceID% /qn /l*v d:\adInstall.log 
IF %ERRORLEVEL%==0 (
...
COPY /Y "%~dp0config.xml" "%DotNetAgentFolder%\Config\config.xml"
