...
namespace Accela.Apps.Shared.OAuth
{
    ...
    public static class TwitterConsumer
    {
        ...
        public static bool IsTwitterConsumerConfigured
        {
            get
            {
                var consumerKey = ConfigurationSettings.Get(ConfigurationConstant.OAUTH_TWITTER_CONSUMER_KEY);
                var consumerSecret = ConfigurationSettings.Get(ConfigurationConstant.OAUTH_TWITTER_CONSUMER_SECRET);
        ...
        private static InMemoryTokenManager ShortTermUserSessionTokenManager
        {
            get
            {
                ...
                if (tokenManager == null)
                {
                    ...
                    {
                        throw new InvalidOperationException("No Twitter OAuth consumer key and secret could be found in configuratoin file.");
                    }
                }
        ...
        /// </returns>
        public static bool TryFinishSignInWithTwitter(out string screenName, out long userId)
        {
            if (System.Web.HttpContext.Current.Request["denied"] != null)
            {
            ...
            userId = long.Parse(response.ExtraData["user_id"]);
        ...
        public static string GetPictureUrl(string screenName)
        {
            string result = String.Format("https://api.twitter.com/1/users/profile_image/{0}", screenName);
            return result;
        }
    }
}
